...
public class AssertWebElement extends AbstractAssert<AssertWebElement, WebElement> {
    ...
    public AssertWebElement isDisplayed(){
        String actualElementText1 = actual.getText();

        logger.info("Checking if element '" + actualElementText1 + "' is displayed");
        isNotNull();

        if(!actual.isDisplayed()){
            failWithMessage("Element was not displayed!");
        }
        logger.info("WebElement '" + actualElementText1 + "' was displayed!");
        return this;
    }
